-- Customer Questions Database Schema
...
CREATE TABLE `customer_questions` (
  ...
  -- Foreign key constraint (assuming you have customer_question_categories table)
  CONSTRAINT `fk_customer_questions_category` 
    FOREIGN KEY (`category_id`) REFERENCES `customer_question_categories` (`id`) 
...
CREATE VIEW `customer_questions_with_answers` AS
SELECT 
  cq.id,
  cq.name,
  cq.category_id,
...
CREATE INDEX `idx_customer_questions_search` ON `customer_questions` (`name`, `delivery_text`, `return_text`);
CREATE INDEX `idx_delivery_answers_value` ON `customer_delivery_answers` (`dollar_value`);
CREATE INDEX `idx_return_answers_value` ON `customer_return_answers` (`dollar_value`);